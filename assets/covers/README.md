# Cover artwork — airan-neocloud-resiliency

![A radio mast, edge compute enclosure and power cabinet beside continuous and interrupted access routes.](social-preview-v1.jpg)
...
Open [repository settings](https://github.com/dimaggi-ai/airan-neocloud-resiliency/settings),
...
Alt text: A radio mast, edge compute enclosure and power cabinet beside continuous and interrupted access routes.
...
Subject: the resilience of a small edge AI site depends on more than one access route. A single small radio mast and one finned graphite compute enclosure share a raised chalk-colored plinth. Two thin matte copper access paths approach the site on separate low rails; one path has one clean physical gap, the other remains continuous. A single plain pale-metal power enclosure is adjacent to the compute box. Three principal objects, two quiet routes, no dramatic disaster scene, no lightning, no satellite, no circles or status lights. Focus on the self-contained edge site.
```
